...
namespace Utils
{
    ...
    inline uint32_t extractBits(uint32_t value, int start, int end)
    {
        int len = end - start + 1;
    ...
    [[noreturn]] inline void illegal(Word instr, const char* why)
    {
    ...
    inline std::string toHex(uint32_t val)
    {
        std::stringstream ss;
        ss << "0x" << std::hex << val;
        return ss.str();
